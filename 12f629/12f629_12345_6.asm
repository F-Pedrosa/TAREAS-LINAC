...
; command line:  C:\JALLIB~1\compiler\jalv2.exe C:\Users\fabian\Desktop\12f629\12f629_12345_6.jal -s C:\JALLIB~1\lib -no-variable-reuse
                                list p=12f629, r=dec
...
v_pin_a1                       EQU 0x0005  ; pin_a1-->gpio_:1
v_pin_a2                       EQU 0x0005  ; pin_a2-->gpio_:2
...
v_wpu_wpu1                     EQU 0x0095  ; wpu_wpu1-->wpu:1
v_wpu_wpu2                     EQU 0x0095  ; wpu_wpu2-->wpu:2
v_wpu_wpu4                     EQU 0x0095  ; wpu_wpu4-->wpu:4
v_wpu_wpu5                     EQU 0x0095  ; wpu_wpu5-->wpu:5
v_ioc                          EQU 0x0096  ; ioc
v_ioc_ioc1                     EQU 0x0096  ; ioc_ioc1-->ioc:1
v_ioc_ioc2                     EQU 0x0096  ; ioc_ioc2-->ioc:2
v_ioc_ioc3                     EQU 0x0096  ; ioc_ioc3-->ioc:3
v_ioc_ioc4                     EQU 0x0096  ; ioc_ioc4-->ioc:4
v_ioc_ioc5                     EQU 0x0096  ; ioc_ioc5-->ioc:5
v_encender6                    EQU 0x0026  ; encender6-->_bitbucket:0
v_tiempo_encendido1            EQU 2
v_tiempo_encendido2            EQU 4
v_tiempo_encendido3            EQU 6
v_tiempo_encendido4            EQU 8
v_tiempo_encendido5            EQU 10
v_encendido                    EQU 0x0025  ; encendido
v__bitbucket                   EQU 0x0026  ; _bitbucket
...
v__floop9                      EQU 0x0027  ; _floop9
v__floop10                     EQU 0x0028  ; _floop10
v___x_23                       EQU 0x005d  ; x-->porta_shadow_:0
v___x_24                       EQU 0x005d  ; x-->porta_shadow_:0
...
;  382    CMCON  = 0b0000_0111
                               ...
                               datalo_clr v_cmcon
                               movwf    v_cmcon
; C:\Users\fabian\Desktop\12f629\12f629_12345_6.jal
;   18 enable_digital_io()                ; todos los pines disponibles como digitales
; C:\JALLIB~1\lib/12f629.jal
;  389    comparator_off()
; C:\Users\fabian\Desktop\12f629\12f629_12345_6.jal
...
l__l108
                               ...
                               movwf    v__pic_temp+1
l__l109
                               branchhi_clr l__l109
                               branchlo_clr l__l109
                               decfsz   v__pic_temp+1,f
                               goto     l__l109
                               branchhi_clr l__l108
                               branchlo_clr l__l108
...
;   28 pin_GP2_direction = INPUT
                               bsf      v_trisio, 2 ; pin_a2_direction
;   30 pin_GP1_direction = INPUT
                               bsf      v_trisio, 1 ; pin_a1_direction
...
; C:\Users\fabian\Desktop\12f629\12f629_12345_6.jal
;   36 salida6 = off
;   38 var bit encender6 = false
                               bcf      v__bitbucket, 0 ; encender6
;   49 OPTION_REG_NGPPU = false
...
;   53 WPU_WPU2 = true
                               bsf      v_wpu, 2 ; wpu_wpu2
;   54 WPU_WPU1 = true
                               bsf      v_wpu, 1 ; wpu_wpu1
;   56 IOC_IOC5 = true
...
;   59 IOC_IOC2 = true
                               bsf      v_ioc, 2 ; ioc_ioc2
;   60 IOC_IOC1 = true
                               bsf      v_ioc, 1 ; ioc_ioc1
;   62 OPTION_REG_INTEDG = false
                               bcf      v_option_reg, 6 ; option_reg_intedg
;   63 INTCON_GPIE = true
                               bsf      v_intcon, 3 ; intcon_gpie
;   64 INTCON_GIE = true
                               bsf      v_intcon, 7 ; intcon_gie
;   67 PROCEDURE rutina_isr() IS
                               goto     l__l87
l_rutina_isr
;   69    if INTCON_GPIF then
                               btfss    v_intcon, 0 ; intcon_gpif
                               goto     l__l90
;   70       INTCON_GPIF = low
                               bcf      v_intcon, 0 ; intcon_gpif
;   71       if entrada1 == low then
                               btfsc    v_gpio_, 5 ; pin_a5
                               goto     l__l92
;   72          encendido = tiempo_encendido1
                               movlw    2
                               movwf    v_encendido
;   73       elsif entrada2 == low then
                               goto     l__l91
l__l92
                               btfsc    v_gpio_, 4 ; pin_a4
                               goto     l__l93
;   74          encendido = tiempo_encendido2
                               movlw    4
                               movwf    v_encendido
;   75       elsif entrada3 == low then
                               goto     l__l91
l__l93
                               btfsc    v_gpio_, 3 ; pin_a3
                               goto     l__l94
;   76          encendido = tiempo_encendido3
                               movlw    6
                               movwf    v_encendido
;   77       elsif entrada4 == low then
                               goto     l__l91
l__l94
                               btfsc    v_gpio_, 2 ; pin_a2
                               goto     l__l95
;   78          encendido = tiempo_encendido4
                               movlw    8
                               movwf    v_encendido
;   79       elsif entrada5 == low then
                               goto     l__l91
l__l95
                               btfsc    v_gpio_, 1 ; pin_a1
                               goto     l__l96
;   80          encendido = tiempo_encendido5
                               movlw    10
                               movwf    v_encendido
;   81       end if
l__l96
...
;   87 forever loop
l__l97
;   88    if encender6 then
                               btfss    v__bitbucket, 0 ; encender6
                               goto     l__l100
;   89       encender6 = false
                               bcf      v__bitbucket, 0 ; encender6
;   90       salida6 = on
                               bsf      v_porta_shadow_, 0 ; x23
; C:\JALLIB~1\lib/12f629.jal
;  131    PORTA_ = PORTA_shadow_
                               movf     v_porta_shadow_,w
                               datalo_clr v_gpio_
