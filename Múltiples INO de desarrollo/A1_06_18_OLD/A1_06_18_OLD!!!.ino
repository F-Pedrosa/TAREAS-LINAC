...
const int intervalo = 30;                    // 5 minutos, valor aleatorio elegido sin mayor implicancia
volatile int cantidad_segundos = 0;
unsigned long currentMillis = 0;
unsigned long previousMillis = 0;
const unsigned long interval = 20000;                       // 20 segundos es el tiempo que le asignamos para esperar el pedido desde el cel.
...
byte wait_for_esp_response(int timeout, char* term=OKrn) {  
  ...
  byte i=0;  
  int len=strlen(term);  
  byte len=strlen(OKrn);  
  // wait for at most timeout milliseconds or if OK\r\n is found  
  while(millis()<t+timeout) {  
     if(Serial.available()) {  
        buffer[i++]=Serial.read();
        if(i>=len) {  
             if(strncmp(buffer+i-len, OKrn, len)==0) {  
             found=true;  
             break;  
           }  
        }  
