// Para probar la A2 (y las otras plaquetas #2, o sea AMENAN2)
...
#define SSID "nodoB2"   // acá el nombre de la red wifi
const char CONSTANTE[3] = "B2"; // el identificador de la plaqueta
...
// Nombres de archivos donde se dejarán las tramas y bitácoras
const char str[] = "full.txt";
const char logfile[] = "bita.txt";
const char str2[] = "alim.txt";
// el pin que cuenta los pulsados
const int PinPulsos = 2;
...
void obtenerFechaHora()
{
  ...
  dia = bcd2Dec(Wire.read());   // lectura dummy para ahorrar un byte de RAM
...
void grabar_datos_archivo()
{
  char buffer1[3];
  char buffer2[8];
  file = SD.open(str, O_CREAT | O_APPEND | O_WRITE);
  obtenerFechaHora();
  file.println("");
  ...
  file.print(buffer1);
  file.print(F(" - "));
  t_h = dht.readTemperature();
  file.print((float)t_h,1); //file.print(F(" *C, "));
  file.print(F(" - "));
  t_h = dht.readHumidity();
  file.print((float)t_h); //file.print(F(" RH%"));
  file.print(F(" - "));
...
void grabar_mensual(void){
  ...
  char buffer2[8];
  SD.chdir("mensual");   
  file = SD.open("0618.txt", O_CREAT | O_APPEND | O_WRITE);
  file.println("");
  file.print(CONSTANTE);
  file.print(F(" "));  
  sprintf(buffer2, "%07d", indice);
...
void recepcion_pedido_wifi(void){
  ...
  if (cuenta == 4){
     ...
     if (mi_cadena == "0000.txt"){      // mandar completo
          ...
          mi_cadena = "bita.txt";      
          mi_cadena.toCharArray(str3, 9);
     }else if (mi_cadena == "6666.txt"){      // mandar bitácora
...
void setup(){
  ...
  // Inicializar I2C
  Wire.begin();
  pinMode(PinPulsos, INPUT_PULLUP);   
  //pin CS a la SD definido como salida
  pinMode(SD_CS,OUTPUT);
  // initializar SPI
  SPI.begin();
  //Inicializar SD, si falla, queda piteando constantemente
  if (!SD.begin(SD_CS)) {
      digitalWrite(LED_ROJO,HIGH);
  }else{
