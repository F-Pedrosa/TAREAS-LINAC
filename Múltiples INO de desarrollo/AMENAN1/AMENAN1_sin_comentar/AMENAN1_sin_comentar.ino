...
// "unsigned long" para variables que son tiempo interno, puesto que están en milisegundos
volatile unsigned long previoMillis = 0;
volatile unsigned long actualMillis = 0;
//const long intervalo = 180000;           // 180 segundos
const long intervalo = 10000;           // 10 segundos para las pruebas
...
void crear_archivo()
{
  ...
  outFile.println(F("   INDICE   FECHA      HORA       TEMP        HUM%   TEMP2   PRES     PULSADOS"));      
...
// Leer sensor de presión y temperatura BMP180
    ...
    outFile.print(F(" *C - "));
    outFile.print(bmp.readPressure());
    outFile.print(F(" Pa - "));
    
    // esta línea se agregó SOLAMENTE para que en las pruebas con la AMENAN1 que le faltaba el sensor de humeda,
    // quedara acomodada legiblemente la trama.
//    outFile.print(F("                     "));
...
void setup()
{
  ...
  previoMillis = millis(); 
  // mandarlo a dormir hasta que lo despierte una activación del pin
...
void loop() 
{
    // Si se despertó, empezar a contar
    digitalWrite(LED_ROJO,HIGH);
    actualMillis = millis();
    // chequear si pasaron INTERVALO segundos... 
    if ((actualMillis - previoMillis) >= intervalo) {
        previoMillis = millis();
        // Deshabilitar momentáneamente la interrupción para evitar molestias durante el procesado importante
        detachInterrupt(0);     
