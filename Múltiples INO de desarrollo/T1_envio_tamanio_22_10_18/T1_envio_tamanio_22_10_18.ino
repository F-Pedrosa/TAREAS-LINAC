...
void recepcion_pedido_wifi(void){
  ...
  while (Serial.available() > 0) {
    inByte = Serial.read();            
    // bloque de archivo completo (full.txt)
    if (inByte == '@'){
...
//    // bloque de archivo bitácora de alimentación (alim.txt)    
//    if (inByte == '$'){
//      while (Serial.available() > 0) {Serial.read();}
//      inByte = ' ';      
//      if (primer_paquete){
...
//      }
//    }  
//    // bloque de archivo bitácora de alimentación en directorio respaldo
//    if (inByte == '^'){
//      while (Serial.available() > 0) {Serial.read();}
//      inByte = ' ';      
//      if (primer_paquete){
...
//      }
//    }  
//    // bloque de archivo completo en directorio respaldo
//    if (inByte == '_'){
//        while (Serial.available() > 0) {Serial.read();}
//        inByte = ' ';      
//        if (primer_paquete){  
...
//    }
//
//    // bloque de borrado (total y parcial)        
//    if (inByte == '~'){
//      while (Serial.available() > 0) {Serial.read();}
//      digitalWrite(LED_ROJO,HIGH);               
//      SD.wipe();
...
//      cerrar;      
//      file = SD.open("full.txt", O_CREAT | O_APPEND | O_WRITE);
//      for (int ix=0; ix <= 5000; ix++){file.print(".");}
//      file.close();
//      delay(500);
//      file = SD.open("alim.txt", O_CREAT | O_APPEND | O_WRITE);
//      for (int ix=0; ix <= 5000; ix++){file.print(".");}
