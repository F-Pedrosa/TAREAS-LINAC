...
const int intervalo = 250;                    // 5 minutos, valor aleatorio elegido sin mayor implicancia
...
void inicializar_wifi(void){
  ...
  Serial.println(F("AT+CWMODE_CUR=3"));
  delay(100);
  Serial.println(F("AT+CWDHCP_CUR=2,1"));
  delay(100);
...
void setup(){
  ...
  delay(500);
  //Serial.println(F("AT+CWDHCP_DEF=0,0"));   
  //delay(500);
  // Inicializar I2C
  Wire.begin();
...
void loop(void) {  
    ...
    Serial.println(F("AT+GSLP=270000"));
    delay(500);  
    for (cantidad_segundos = 0 ;  cantidad_segundos  < 250 ; cantidad_segundos++){
...
//    grabar_alimentacion();
    ...
    salir = false;   
    // esperar pedido de conexión desde el celular durante "intervalo" segundos (50 por el momento).
    previousMillis = millis();
    while (salir == false) {  
